...
nmb = [re.sub('^(.+)', r'<http://kanpsack/Activities/\1>\t<http://www.w3.org/2000/01/rdf-schema#label>', x) for x in nmb]

# 指定列の取り出し02
cal1 = list(map(lambda line: line.split('\t')[0], lines))

pattern22=re.compile(r'(^.+)') 
cal1 = [re.sub('^(.+)', r'"\1"@en ;\t<http://kanpsack/property/sp2>', x) for x in cal1]
...
lines_ttl = ["{0}\t{1}\t{2}".format(line0, line1, line2) for line0, line1, line2 in zip(nmb, cal1, cal2)]

#.joinで，改行'\n'で行を連結, .joinで，空文字列''で行を単純連結
save_file('Activity_Species_fromNatural_Activity_act_list.ttl', "\n".join(lines_ttl)) # .joinで，改行'\n'で行を連結
